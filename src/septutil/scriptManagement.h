#if !defined(SCRIPTMANAGEMENT_H_INCLUDED2)
#define SCRIPTMANAGEMENT_H_INCLUDED2
...
namespace ScriptManagement {
    ...
    inline CVirtualMachine_ExecuteScript_t CVirtualMachine_ExecuteScript =
    ...
    inline CVirtualMachine_ExecuteScriptEnhanced_t CVirtualMachine_ExecuteScriptEnhanced =
    ...
    inline CVirtualMachine_InitParam_t CVirtualMachine_InitParam =
        std::_Bit_cast<CVirtualMachine_InitParam_t>(NWN2_OFFSET_InitParam);
    inline CVirtualMachine_CleanParam_t CVirtualMachine_CleanParam =
        std::_Bit_cast<CVirtualMachine_CleanParam_t>(NWN2_OFFSET_CleanParam);

    inline CVirtualMachine*
    ...
    inline void AddFParam(uint32_t* ptrParams, float fParam)
    {
        //float Param
    ...
    inline void AddParam(uint32_t* ptrParams, float fParam)
    {
        AddFParam(ptrParams, fParam);
    }

    inline void AddParam(uint32_t* ptrParams, double dParam)
    {
        AddFParam(ptrParams, dParam);    
    }


    inline void AddParam(uint32_t* ptrParams, NWN::OBJECTID oParam)
    {
        //NwnObject Param
    ...
    inline void AddParam(uint32_t* ptrParams, const char* sParam)
    {
    ...
    inline void AddParam(uint32_t* ptrParams, int iParam)
    {
        //Int Param
    ...
    inline void AddParameters(uint32_t* ptrParams){}

    template<typename T, typename... Targs>
    inline void AddParameters(uint32_t* ptrParams,T value,  Targs... Fargs)
    {
    ...
    inline void PrepareCleanParameter(uint32_t* ptrParams, int iSize)
    {
...
int ExecuteEnhancedScript_sp(std::string sScriptName, NWN::OBJECTID oTarget, int& iRetValue, Targs... Parameters)
{
    int retValue = -1;
    ...
    //call the script (get back validity here too ?)
    ...
    ScriptManagement::CVirtualMachine_CleanParam(param1); //Probl�me si string ?
