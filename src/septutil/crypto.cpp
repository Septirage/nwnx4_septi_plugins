...
void ShiftRowsI(unsigned char * state) {
	unsigned char tmp[16];
...
*/
void RoundI(unsigned char * state, unsigned char * key) {
...
void AESDecrypt(unsigned char * encryptedMessage, unsigned char * expandedKey, unsigned char * decryptedMessage)
{
	...
	for (int i = 8; i >= 0; i--) {
		RoundI(state, expandedKey + (16 * (i + 1)));
	}
...
{
	long long* encrypted = new long long[message_size];// malloc(sizeof(long long) * message_size);
	if(encrypted == NULL){
		fprintf(stderr,
